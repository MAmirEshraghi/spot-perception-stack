#!/usr/bin/env python3
"""
Fast SAM Segmentation Helper
Simple box prompt mode: one Fast SAM call per image with multiple box prompts.

Author: Robin Eshraghi 
initial 12/15/25

(12/18/25) Updated to use the new matching function, and benckmarking. 
            

Usage: "python fast_sam_helper.py --real --viz --profile"
                    --real: use real data
...
Output data path:           /logs/current_run_outputs/offline_outputs/fast_sam_helper_benchmark
Output visualization path:  /logs/current_run_outputs/offline_outputs/fast_sam_helper_benchmark/visualizations
"""
...
def _visualize_benchmark_result(image: np.ndarray, bboxes: List[np.ndarray], 
    ...
    title_text = "Matching: YOLO bbox and FastSAM mask"
    title_scale = 0.7
    title_thickness = 2
    ...
    # Blue: unmatched FastSAM mask and box
    cv2.rectangle(vis_with_legend, (10, y_offset + 15), (30, y_offset + 30), (0, 0, 255), -1)
    cv2.putText(vis_with_legend, "Blue: unmatched FastSAM mask and its box", (35, y_offset + 30), font, font_scale, (0, 0, 0), thickness)
    
    # Green: matched ones
    cv2.rectangle(vis_with_legend, (10, y_offset + 40), (30, y_offset + 55), (0, 150, 0), -1)
    cv2.putText(vis_with_legend, "Green: matched YOLO bbox and FastSAM mask box", (35, y_offset + 55), font, font_scale, (0, 0, 0), thickness)
...
def get_fastsam_masks_from_boxes(
    ...
    Args:
        ...
        retina_masks: Use retina masks (default: True)
        config: Optional config dict with 'conf', 'iou', 'imgsz', 'retina_masks', 'device' (overrides defaults if provided)
    ...
    if config is not None:
        ...
        retina_masks = config.get("retina_masks", retina_masks)
    ...
    try:
        ...
        print(f"Warning: FastSAM inference failed: {e}")
        masks.extend(_create_empty_masks(len(bboxes), (H, W)))
...
def run_benchmark(model_path: str = "FastSAM-x.pt", num_iterations: int = 10, 
    ...
    if not FASTSAM_AVAILABLE:
        print("ERROR: FastSAM not available")
        return
    
    print(f"\n{'='*70}")
    print(f"FastSAM Benchmark")
    print(f"{'='*70}")
    ...
    if enable_viz:
        ...
        viz_dir = project_root / "logs" / "current_run_outputs" / "offline_outputs" / "fast_sam_helper_benchmark"
    ...
    _ = get_fastsam_masks_from_boxes(model, test_image, test_bboxes, device=device, config=config)
    ...
    for idx, (test_image, test_bboxes) in enumerate(test_cases):
        start = time.perf_counter()
        masks = get_fastsam_masks_from_boxes(model, test_image, test_bboxes, device=device, config=config)
